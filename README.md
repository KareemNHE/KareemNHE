👋 Hi there, I'm Kareem Eid
I'm a final-year Computer Science student at Sheffield Hallam University, passionate about crafting innovative software solutions that address real-world challenges. My journey in technology is driven by a commitment to continuous learning and a desire to make impactful contributions.

<b>💻 Technical Skills:</b>

Languages: C++, C#, Python, Dart, JavaScript, PHP​

Web Development: HTML, CSS, JavaScript, PHP, REST APIs​

Frameworks: Laravel, React Native, Vue.js​

Mobile Development: Flutter, Dart, Firebase​

Databases: MySQL, SQLite, Firebase​

AI & Machine Learning: Python (TensorFlow, NumPy, Pandas)​

Other: Agile/Scrum methodologies, Project coordination, Team Leadership​

🚀 Projects:

Online Library System: Developed a platform allowing users to rent media for three weeks, incorporating subscription, payment, and profile functionalities.​

Hospital Navigation System: Created a GPS-based site navigation tool with video walkthroughs for Chesterfield Royal Hospital, receiving excellent feedback.​

Sports Social Media App: Built a Flutter and Firebase-powered app integrating camera functionalities and real-time notifications.​

AI Image Caption Generator: Engineered a Python-based tool utilizing deep learning to generate captions for images.​

🏆 Achievements:

Trampolining: Ranked 14th in the UK at the 2023 English Championships; accumulated 19 medals and 2 trophies in DMT events.​

Academic Awards: Received Art and Maths awards in secondary school for excellence in digital painting and mathematical proficiency.​

Professional Recognition: Consistently exceeded sales targets at River Island and Hollister, earning praise as a valuable employee.​

Community Engagement: Assisted seven special needs children in advancing their Arabic studies, contributing to their academic success.​

🎯 Interests:

Fitness & Strength Training: Maintain a rigorous gym regimen, achieving personal bests of 160kg bench press, 200kg squat, and 230kg deadlift.​

Cooking: Enjoy experimenting with new recipes and exploring diverse cuisines.​

Travel & Culture: Lived in Egypt, Saudi Arabia, and Dubai, fostering a deep appreciation for cultural diversity.​

Gaming & Football: Engage in both solo and team-based gaming; play casual football with friends.​

Feel free to connect with me or explore my repositories to see my work in action!
